...
**flexBART** now supports fitting heteroskedastic models with Bayesian
ensembles of regression trees. Specifically, it can be used to fit
models of the form \\ Y \sim \mathcal{N}(\mu(x), \sigma(x)^2) \\ where
both \\\mu(X)\\ and \\v(x)\\ are approximated with an ensemble of
...
from the model \\Y \sim \mathcal{N}(\mu(x), \sigma(x)^2)\\ where \\X\\
contains a single continuous covariate.

We generate \\n = 1000\\ training observations and \\n = 101\\ testing
observations.
...
returns the posterior mean estimate and posterior samples for \\\mu(x)\\
(outputs named `yhat`) and \\\sigma(x)\\ (outputs named `sigma`). We
plot the true underlying \\\mu(x)\\ and \\\sigma(x)\\ functions along
with the mean and 95% credible intervals estimated by
[`flexBART::flexBART`](https://skdeshpande91.github.io/flexBART/reference/flexBART.md).
...
![](heteroskedastic_model_files/figure-html/plot-mu-sigma-1.png)
