...
fit models of the form \\ Y = \beta\_{0}(X) + \beta\_{1}(X) \times
Z\_{1} + \cdots + \beta\_{R-1}(X) \times Z\_{R-1} + \sigma \epsilon \\
where each function \\\beta\_{r}(X)\\ is approximated with an ensemble
of regression trees. **flexBART** allows the size (specified using the
...
to vary in each ensemble. It further allows for
redundancy/non-identifiability in which some of the \\Z\_{r}\\’s are
exactly equal to one another. This is useful if one wishes to fit, say,
an additive model with BART (e.g., \\Y = f\_{1}(X\_{1}) +
f\_{2}(X\_{2})\\ with BART priors on \\f\_{1}\\ and \\f\_{2}\\).

## Identified example

We first demonstrate **flexBART**’s functionality using data generated
from the model \\Y = \beta\_{0}(X) + \beta\_{1}(X)Z\_{1} +
\beta\_{2}(X)Z\_{2} + \sigma\epsilon\\ where \\X\\ contains
\\p\_{\text{cont}} = 10\\ continuous and \\p\_{\text{cat}} = 10\\
categorical covariates. Each categorical covariate will take on 10
different values.

We generate \\n = 10000\\ training observations and \\n = 5000\\ testing
observations.
...
response surface (outputs named `yhat`). We first plot the posterior
...
![](varying_coefficients_files/figure-html/plot-fits-vcbart1-1.png) \###
Pointwise uncertainty We can also assess the coverage of the pointwise
credible intervals
...
for which this probability exceeds 50%. The `varcounts` element of
[`flexBART::flexBART`](https://skdeshpande91.github.io/flexBART/reference/flexBART.md)’s
...
We see that \\X\_{1}, X\_{2}\\ and \\X\_{11}\\ are selected for
\\\beta\_{0};\\ \\X\_{1}\\ is selected for \\\beta\_{2};\\ and
\\X\_{1}\\ is selected for \\\beta\_{3}.\\ So, at least for this run,
there is one false positive and no false negative identifications
...
the \\Z\_{r}\\’s are identical. The formula parser in
[`flexBART::flexBART`](https://skdeshpande91.github.io/flexBART/reference/flexBART.md)
...
To illustrate, suppose we generate data from the redundant model \\ Y =
\[\beta\_{0}(X) + \beta\_{1}(X\] + \[\beta\_{2}(X) + \beta\_{3}(X)\]
\times Z\_{1} + \[\beta\_{4}(X) + \beta\_{5}(X)\] \times Z\_{2} + \sigma
\epsilon. \\ If we specify a formula like

    Y~bart(.) + bart(.) + Z1*bart(.) + Z1*bart(.) + Z2*bart(.) + Z2*bart(),

[`flexBART::flexBART`](https://skdeshpande91.github.io/flexBART/reference/flexBART.md)
will return posterior samples for 6 ensembles on the standardized scale.
These samples are saved in the `raw_beta` slot of the output. As
written, although the functions \\\beta\_{0}\\ and \\\beta\_{1}\\ are
not identified, their sum is. So,
[`flexBART::flexBART`](https://skdeshpande91.github.io/flexBART/reference/flexBART.md)
...
We now generate \\X\\ and \\Z\\ values and evaluate the true slopes and
...
![](varying_coefficients_files/figure-html/plot-vcbart2-1.png)
