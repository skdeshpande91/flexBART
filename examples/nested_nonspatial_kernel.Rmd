...
nested$School <- floor(train_data$Classroom/5)
nested$District <- floor(train_data$School/6)

nested$Classroom <- factor(train_data$Classroom, levels = 0:119)
nested$School <- factor(train_data$School, levels = 0:23)
nested$District <- factor(train_data$District, levels = 0:3)
```
...
Nevertheless, we see that the co-clustering probabilities clearly exhibit the hierarchical relationship
<!--
...
For `nest_v_option = 1`, the initial splitting variable is always `District`. Subsequently, the tree can split on any of the other variables as both `Classroom` and `District` are nested within `District`
...
To verify that the code is doing this correctly: 
```{r sanity-check-2}
any(partition2$varcounts[which(partition2$num_leafs > 1),1] == 0)
```
Finally, for `nest_v_option = 3`, 
