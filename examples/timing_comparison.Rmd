...
**flexBART** is often much faster than **BART** because it avoids a lot of redundant computations.
The speedup really manifests when you have lots of data.
To illustrate, let's look compare the two implementations on an easy task (or at least, easy for BART): a slightly modified version of the [Friedman function](https://www.sfu.ca/~ssurjano/fried.html)!
```{r define_friedman}
mu_true <- function(df){
  if(!all(abs(df) <= 1)){
...
```
Note: the Friedman function is defined over the unit square but **flexBART** assumes that continuous predictors lie in [-1,1].
...
```
To make the timing comparison fair, we'll run 4 `BART::wbart` chains for the same number of iterations.
