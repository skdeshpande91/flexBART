...
Rcpp::List sn_fit(Rcpp::NumericVector Y_train,
                  ...
                  double sigest, double lambda, double nu,int M,
                  ...
                  bool verbose, int print_every)
{
  ...
  if(R != 1) Rcpp::stop("[single_unnested_fit]: this function assumes R = 1");
  int p_cont = 0;
  int p_cat = 0;
  ...
  for(int iter = 0; iter < total_draws; iter++){
    ...
    for(int m = 0; m < M; ++m){
      ...
      update_tree_nested(t_vec[m], ss_train_vec[m], ss_test_vec[m], accept, r, sigma, di_train, di_test, tree_pi, gen); // update the tree
      total_accept += accept;
