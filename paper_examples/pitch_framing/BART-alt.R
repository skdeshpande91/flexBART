# BART simulation which does not drop constant columns
library(BART)
library(dbarts)
...
assign(paste0("BART_alt_", year, "_", fold),
       list(year = year, fold_num = fold_num,
            timing = timing,
            misclass = misclass,
            brier = brier,
            logloss = logloss))
save(list = paste0("BART_alt_", year, "_", fold),
     file = paste0("BART_alt_", year, "_", fold, ".RData"))
