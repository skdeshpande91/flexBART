...
timing <- 
  system.time(
    fit <- flexBART::probit_flexBART(Y_train = Y_train,
                                     X_cont_train = X_cont_train, X_cat_train = X_cat_train,
                                     X_cont_test = X_cont_test, X_cat_test = X_cat_test,
                                     unif_cuts = unif_cuts, cutpoints_list = cutpoints_list,
                                     cat_levels_list = cat_levels_list, a_cat = 0, b_cat = 0,
...
assign(paste0("flexBART_", year, "_", fold_num),
       list(year = year, fold_num = fold_num,
            timing = timing,
            misclass = misclass,
            brier = brier,
            logloss = logloss))
save(list = paste0("flexBART_", year, "_", fold_num),
     file = paste0("flexBART_", year, "_", fold_num, ".RData"))
