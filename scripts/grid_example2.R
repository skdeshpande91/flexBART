...
# let's generate 5 observations per spatial unit
Y1 <- mu + sigma * rnorm(n, 0, 1)
Y2 <- mu + sigma * rnorm(n, 0, 1)
Y3 <- mu + sigma * rnorm(n, 0, 1)
Y4 <- mu + sigma * rnorm(n, 0, 1)
Y5 <- mu + sigma * rnorm(n, 0, 1)


Y_all <- c(Y1, Y2, Y3, Y4, Y5)
...
X_cat1 <- matrix(as.integer(1:n) - 1, nrow = n, ncol = 1)
X_cat2 <- X_cat1
X_cat3 <- X_cat1
X_cat4 <- X_cat1
X_cat5 <- X_cat1
...
X_cat_all <- rbind(X_cat1, X_cat2, X_cat3, X_cat4, X_cat5) # note C++ is 0-indexed

# For comparison with/ regular BART we need to also
# represent each spatial unit with a dummy variable
X_cont1 <- matrix(0, nrow = n, ncol = n)
diag(X_cont1) <- 1
X_cont2 <- X_cont1
X_cont3 <- X_cont1
X_cont4 <- X_cont1
X_cont5 <- X_cont1
X_cont_all <- rbind(X_cont1, X_cont2, X_cont3, X_cont4, X_cont5)
...
# tmp_test_index refer to the sites that will be held out of training
# we have to stack the appropriate design matrices

test_index <- c(tmp_test_index, tmp_test_index + n, tmp_test_index + 2 * n, 
                tmp_test_index + 3 * n, tmp_test_index + 4 * n)
train_index <- c(tmp_train_index, tmp_train_index + n, tmp_train_index + 2 * n, 
                 tmp_train_index + 3 * n, tmp_train_index + 4 * n)
...
# In-sample RMSE:
mu_all <- c(mu, mu, mu, mu, mu)

# because I was lazy in passing data to the methods, 
# we have repeated predictions 5 times
sqrt(mean( (mu_all[train_index] - post_mean_train1)^2 )/5) 
sqrt(mean( (mu_all[train_index] - post_mean_train2)^2 )/5)
sqrt(mean( (mu_all[train_index] - post_mean_train3)^2 )/5)
sqrt(mean( (mu_all[train_index] - post_mean_train4)^2 )/5)

# Out-of-sample RMSE
sqrt(mean( (mu_all[test_index] - post_mean_test1)^2 )/5)
sqrt(mean( (mu_all[test_index] - post_mean_test2)^2 )/5)
sqrt(mean( (mu_all[test_index] - post_mean_test3)^2 )/5)
sqrt(mean( (mu_all[test_index] - post_mean_test4)^2 )/5)
...
mean( (mu_all[test_index] - post_mean_test1)^2 ) / mean( (mu_all[test_index] - y_mean)^2 )
mean( (mu_all[test_index] - post_mean_test2)^2 ) / mean( (mu_all[test_index] - y_mean)^2 )
mean( (mu_all[test_index] - post_mean_test3)^2 ) / mean( (mu_all[test_index] - y_mean)^2 )
mean( (mu_all[test_index] - post_mean_test4)^2 ) / mean( (mu_all[test_index] - y_mean)^2 )
...
plot("figures/grid_example2.png", width = 8, height = 6, units = "in", res = 600)
...
g_data <- g
scaled_Y <- scales::rescale(Y1, to = c(0,1), from = max_value * c(-1,1))
V(g_data)$color <- rgb(colorRamp(col_list, bias = 1)(scaled_Y)/255)
plot(g_data, layout = layout_on_grid, main = "Y1", vertex.label = "")

g_train <- g
V(g_train)$color <- rgb(colorRamp(col_list, bias = 1)(scaled_Y)/255)
V(g_train)$color[tmp_test_index] <- "darkgray" 
plot(g_train, layout = layout_on_grid, main = "Training sites", vertex.label = "")

# Let's plot the original data

# Fit 2
g_fit2 <- g
scaled_fit2_train <- scales::rescale(post_mean_train2[1:n_train], to = c(0,1), from = max_value * c(-1,1))
scaled_fit2_test <- scales::rescale(post_mean_test2[1:n_test], to = c(0,1), from = max_value * c(-1,1))
V(g_fit2)$color <- rep(NA, times = n)
V(g_fit2)$color[tmp_train_index] <- rgb(colorRamp(col_list, bias = 1)(scaled_fit2_train)/255)
V(g_fit2)$color[tmp_test_index] <- rgb(colorRamp(col_list, bias = 1)(scaled_fit2_test)/255)
plot(g_fit2, layout = layout_on_grid, main = "flexBART (w/ adjacency)", vertex.label = "")


g_fit3 <- g
scaled_fit3_train <- scales::rescale(post_mean_train3[1:n_train], to = c(0,1), from = max_value * c(-1,1))
scaled_fit3_test <- scales::rescale(post_mean_test3[1:n_test], to = c(0,1), from = max_value * c(-1,1))
V(g_fit3)$color <- rep(NA, times = n)
V(g_fit3)$color[tmp_train_index] <- rgb(colorRamp(col_list, bias = 1)(scaled_fit3_train)/255)
V(g_fit3)$color[tmp_test_index] <- rgb(colorRamp(col_list, bias = 1)(scaled_fit3_test)/255)
plot(g_fit3, layout = layout_on_grid, main = "flexBART (w/o adjacency)", vertex.label = "")

g_fit4 <- g
scaled_fit4_train <- scales::rescale(post_mean_train4[1:90], to = c(0,1), from = max_value * c(-1,1))
scaled_fit4_test <- scales::rescale(post_mean_test4[1:10], to = c(0,1), from = max_value * c(-1,1))
V(g_fit4)$color <- rep(NA, times = n)
V(g_fit4)$color[tmp_train_index] <- rgb(colorRamp(col_list, bias = 1)(scaled_fit4_train)/255)
V(g_fit4)$color[tmp_test_index] <- rgb(colorRamp(col_list, bias = 1)(scaled_fit4_test)/255)
