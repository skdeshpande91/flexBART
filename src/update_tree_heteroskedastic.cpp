...
void compute_ss_grow_heteroskedastic_multi(suff_stat &ss, std::map<int, jump_post> &jp_map, int &nx_nid, rule_t &rule, int &r, data_info &di, tree_prior_info &tree_pi)
{
  int i = 0;
  double z = 0.0;
  double sigma = 0.0;
...
void grow_tree_heteroskedastic_single(tree &t, suff_stat &ss_train, suff_stat &ss_test, std::map<int, jump_post> &jp_map, int &accept, data_info &di_train, data_info &di_test, tree_prior_info &tree_pi, RNG &gen)
{
  ...
  compute_ss_grow_heteroskedastic_single(ss_train, jp_map, nx_nid, rule, di_train, tree_pi);
  int nxl_nid = 2*nx_nid;
  int nxr_nid = 2*nx_nid+1;
...
void grow_tree_heteroskedastic_multi(tree &t, suff_stat &ss_train, suff_stat &ss_test, std::map<int, jump_post> &jp_map, int &accept, int &r, data_info &di_train, data_info &di_test, tree_prior_info &tree_pi, RNG &gen)
{
  ...
  compute_ss_grow_heteroskedastic_multi(ss_train, jp_map, nx_nid, rule, r, di_train, tree_pi);
  int nxl_nid = 2*nx_nid;
  int nxr_nid = 2*nx_nid+1;
...
void prune_tree_heteroskedastic_single(tree &t, suff_stat &ss_train, suff_stat &ss_test, std::map<int, jump_post> &jp_map, int &accept, data_info &di_train, data_info &di_test, tree_prior_info &tree_pi, RNG &gen)
{
  ...
  compute_ss_prune_heteroskedastic_single(ss_train,jp_map, nxl_nid, nxr_nid, nx_nid, di_train, tree_pi);
...
void prune_tree_heteroskedastic_multi(tree &t, suff_stat &ss_train, suff_stat &ss_test, std::map<int, jump_post> &jp_map, int &accept, int &r, data_info &di_train, data_info &di_test, tree_prior_info &tree_pi, RNG &gen)
{
  ...
  int nxl_nid = nxl->get_nid(); // id for nxl
  int nxr_nid = nxr->get_nid(); // id for nxr
  compute_ss_prune_heteroskedastic_multi(ss_train,jp_map, nxl_nid, nxr_nid, nx_nid, r, di_train, tree_pi);
...
void update_tree_heteroskedastic_single(tree &t, suff_stat &ss_train, suff_stat &ss_test, int &accept, data_info &di_train, data_info &di_test, tree_prior_info &tree_pi, RNG &gen)
{
  ...
  compute_jump_posterior_heteroskedastic_single(jp_map, ss_train, di_train, tree_pi);
  
  if(gen.uniform() < PBx) grow_tree_heteroskedastic_single(t, ss_train, ss_test, jp_map, accept, di_train, di_test,tree_pi, gen);
  else prune_tree_heteroskedastic_single(t, ss_train, ss_test, jp_map, accept, di_train, di_test, tree_pi, gen);
...
void update_tree_heteroskedastic_multi(tree &t, suff_stat &ss_train, suff_stat &ss_test, int &accept, int &r, data_info &di_train, data_info &di_test, tree_prior_info &tree_pi, RNG &gen)
{
  ...
  std::map<int, jump_post> jp_map;
  compute_jump_posterior_heteroskedastic_multi(jp_map, ss_train, r, di_train, tree_pi);
  
  if(gen.uniform() < PBx) grow_tree_heteroskedastic_multi(t, ss_train, ss_test, jp_map, accept, r, di_train, di_test,tree_pi, gen);
  else prune_tree_heteroskedastic_multi(t, ss_train, ss_test, jp_map, accept, r, di_train, di_test, tree_pi, gen);
