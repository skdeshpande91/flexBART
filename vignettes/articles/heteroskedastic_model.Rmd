...
```
We see that $X_{1}, X_{2}$ and $X_{11}$ are selected for $\beta_{0};$ $X_{1}$ is selected for $\beta_{2};$ and $X_{1}$ is selected for $\beta_{3}.$
So, at least for this run, there is one false positive and no false negative identifications
...
$$
Y = [\beta_{0}(X) + \beta_{1}(X] + [\beta_{2}(X) + \beta_{3}(X)] \times Z_{1} + [\beta_{4}(X) + \beta_{5}(X)] \times Z_{2} + \sigma \epsilon.
$$
...
These samples are saved in the `raw_beta` slot of the output.
