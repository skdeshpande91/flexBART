...
p_train <- 1/(1 + exp(-mu_train))

mu_test <- 
  beta0_test + test_data[,"Z1"] * beta1_test + test_data[,"Z2"] * beta2_test

p_test <- 1/(1 + exp(-mu_test))

train_data[,"Y"] <-
  rbinom(n = length(mu_train), size = 1, p = p_train)
...
set.seed(99)
fit <-
  flexBART::logit_flexBART(formula = Y ~ bart(.) + Z1 * bart(.) + Z2 * bart(.),
...
plot(p_train, fit$prob.train.mean, pch = 16, cex = 0.5,
...
plot(p_test, fit$prob.test.mean, pch = 16, cex = 0.5,
...
coverage["train", "p"] <-
  mean(p_train >= p_train_l95 & p_train <= p_train_u95)
...
coverage["test", "p"] <-
  mean(p_test >= p_test_l95 & p_test <= p_test_u95)
coverage["test", "beta0"] <-
