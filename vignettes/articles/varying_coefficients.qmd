...
As we did in the [Getting Started article](/vignettes/articles/flexBART.qmd), we can compute the marginal posterior probability inclusion probability that each $X_{j}$ is used in at least one splitting rule in the ensemble for each $\beta_{r}(\boldsymbol{\mathbf{X}}).$
