...
## Overview
**flexBART** supports fitting varying coefficient models with Bayesian ensembles of regression trees.
...
$$Y = \beta_{0}(X) + \beta_{1}(X)Z_{1} + \beta_{2}(X)Z_{2} + \sigma\epsilon,$$ 
where $X$ contains $p_{\text{cont}} = 10$ continuous and $p_{\text{cat}} = 10$ categorical covariates.
...
The three coefficient functions $\beta_{0}(X), \beta_{1}(X),$ and $\beta_{2}(X)$ are defined below.
```{r}
#| label: define-beta
#| eval: true
#| echo: true
...
#| fig-height: 4
#| fig-align: center
#| fig-cap: Actual (horizontal) vs Estimated (vertical) evaluations of $E[Y \vert X,Z]$ and the individual coefficient functions $\beta_{0}(X), \beta_{1}(X),$ and $\beta_{2}(X).$
...
We can form posterior credible intervals for evaluations of the response surface $\E[Y \vert X, Z]$ at each training (resp. testing) observation by computing quantiles in each column of `yhat.train` (resp. `yhat.test`).
...
To form credidble intervals for each $\beta_{j}(\boldsymbol{\mathbf{x}}_{i})$, we must compute column-wise quantiles in the corresponding slice of `beta.train` or `beta.test`.
```{r}
...
Following the recommendations from Section 3.2 of that paper, we can compute the marginal posterior probability inclusion probability that each $X_{j}$ is used at least once in the ensemble for each $\beta_{r}(X).$
...
The dimensions of this array, respectively, index MCMC draws, covariates (i.e., elements of $\boldsymbol{\mathbf{X}}$), and ensembles/covariate functions (i.e., $\beta_{0}, \ldots, \beta_{R-1}$).
```{r varcounts-vcbart1}
dim(fit$varcounts)
```
...
To do so, we first define a new data frame containing a fine grid of $X_{1}$ values, two different $X_{11}$ values (which induce different $\beta_{0}(X)$ values), and dummy values for the remaining predictors and covariates.
```{r}
